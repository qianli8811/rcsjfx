	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$("#khjm").blur(function(){
				...
				$.ajax({
					type: "POST",
					url: "${ctx}/kerz/keRz/getRckhzl",
					dataType: "json",
					data:{"khjm":khjm},
					success: function(data){
						if(data.resultFlag != 1){
							top.layer.alert('客户简码错误，请重新输入', {
								skin: 'layui-layer-molv' //样式类名
								,closeBtn: 0
							});
						}else{
							console.info(data);
							$("#rcKhzlId").val(data.rcKhzlId);
						}
					}
				});
...
<body class="hideScroll">
		<form:form id="inputForm" modelAttribute="keRz" action="${ctx}/kerz/keRz/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   ...
		   <tr>
			   ...
			   <td class="width-35">
				   ...
				   <input name = "rckhzl.khjm" id="khjm" value="${rcKhzl.khjm}" htmlEscape="false"    class="form-control required"/>
			   </td>
