...
$(function(){
	$.ajax({
		type: "POST",
		url: "${ctx}/kerz/rcKhzl/getCityNum",
		dataType: "json",
		success: function(data){
			...
			var convertData = function (data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(dataName.indexOf("市")){
						dataName = dataName.replace("市", "");
					}
					if(dataName.indexOf("县")){
						dataName = dataName.replace("县", "");
					}
					for(var j=0;j<geoCoordMap.length;j++){
						var children = geoCoordMap[j].children;
						if(children){

							for(var k = 0;k<children.length;k++){
								if(dataName == children[k].name){
									var arr = new Array();
									arr[0] = Number(children[k].log);
									arr[1] = Number(children[k].lat);
									arr[2] = dataValue;
									res.push({
										name: dataName,
										value: arr
			...
			var convertedData = [
				convertData(dataArr),
				convertData(dataArr.sort(function(a, b) {
			...
			var legend = '融创金控已经帮助中国'+dataArr.length+'家中小企业点亮融资难题';
...
<body class="gray-bg">
    ...
    <div class="wrapper wrapper-content">
	    <div id="main" style="width: 99%; height: 550px;border: 1px solid #ccc; padding: 10px;"></div>
    </div>
</body>
</html>